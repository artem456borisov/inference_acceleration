...
def _batch_encode(prompts, tokenizer, prompt_len):
...
def batch_infer(model, tokenizer, prompts, batch_size, prompt_len, generate_kwargs = ""):
    answers = []
    for batch_input in tqdm(batch_split(prompts, batch_size)):
        timer.start(sync_func=get_accelerator().synchronize)
        with torch.no_grad():
            encode_inputs = input_tokens = _batch_encode(batch_input, tokenizer, prompt_len)
            outputs = model.generate(**encode_inputs, **generate_kwargs)
